...
def run():
    ...
    for i in range(0, 9):
        print_results(test_sents[i], tagger, indwords_list)
...
def sent_labels(sent):
    return [str(p_label)[0] for token, ner, postag, p_label, i_label, o_label in sent]
...
def combine_spans(spans):
    ...
    for i in range(1, len(spans)-1):
        span_next = spans[i]
        if int(span_next[0])-int(span_prev[1]) <= 4:
            span_new = [span_prev[0], span_next[1]]
            if span_new not in fin_list:
                fin_list.append(span_new)
            span_prev = span_new
        else:
            fin_list.append(span_prev)
            fin_list.append(span_next)
            span_prev = span_next
    return (fin_list)
        
def print_results(example_sent, tagger, indwords_list):
...
def get_training_data():
    #4 is prec, 3 is prec/hand, 5 is fmeasure
    #f = open('/Users/romapatel/Desktop/difficulty_crf_pattern.json', 'r')
    f = open('/Users/romapatel/Desktop/difficulty_crf_pattern3.json', 'r')
    #f = open('/Users/romapatel/Desktop/difficulty_crf_pattern4.json', 'r')
...
def get_train_test_sets():
    ...
    for docid in doc_dict:
        sents = doc_dict[docid]['pattern']['Participants']
