...
def get_results(test_sents, tagger, indwords_list, patterns_list):
    f1 = open(path + set_path + annotype + '-test_pred.json', 'w+')
    f2 = open(path + set_path + annotype + '-test_correct.json', 'w+')
...
def get_patterns():
    ...
    for annotype in annotypes:
        ...
        filename = annotype.lower() + '_bigrams3.tsv'
...
def isindword(word, annotype, indwords_list):
    ...
    for line in f:
        if line[:-1] in word.lower():
            return True
    f = open(path + 'crf_files/disease_names.txt', 'r')
    for line in f:
        if line[:-1] in word.lower():
            return True
